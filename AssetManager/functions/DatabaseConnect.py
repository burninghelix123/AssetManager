'''Connects to a database accepts storing and loading from a file'''
...
def loginPrep():
    ...
    if os.path.exists(tempLocation):
        input = MessageDialog.Dialog('Use Stored Login Info?',
                                     'Would you like to use the login info stored previously?',
                                     'Yes', 'No')
        input = input.exec_()
        if input == 0:
...
def loginToDB():
    ...
    if not username == None:
        input = MessageDialog.Dialog('Store Login Info:',
                                     'Would you like to store the login info for the server?',
                                     'Yes', 'No')
        MessageDialog.Dialog.msgBox.connect(MessageDialog.Dialog.btnYes,
                                            QtCore.SIGNAL('clicked()'),
                                            lambda : storeLogin(username, address, password, databasename))
        input.exec_()
...
def dontUseStored(connected):
    '''Skips reading login info from file'''
    username, address, password, databasename = loginToDB()
    connected, cur, ex = connect(username, address, password, databasename)
    return(connected, cur, ex)
